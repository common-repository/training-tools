 ...
 * @version 1.0
 */  
class TRAININGTOOLS_Formbuilder extends TRAININGTOOLS_UI {
    ...
    public function __construct() {
        ...
        $this->form_val_arr = get_option( 'trainingtools_formvalidation' );
        
        // re-populate class var with the localized value
        $this->first_switch_label = __( 'Enabled', 'trainingtools' );
        $this->second_switch_label = __( 'Disabled', 'trainingtools' );
    }
    ...
    */
    public function input( $formid, $inputtype, $inputid, $inputname, $optiontitle, $titleatt = '', $required = false, $currentvalue = null, $atts_array = array(), $validation_array = array() ) {
        ...
        if( !in_array( $inputtype, $this->input_types, false ) ) {
            self::option_subline( __( 'This is a technical matter please report it. There should be a form input here that you may require before you continue.', 'trainingtools' ), __( 'Invalid Input Type', 'trainingtools' ) );
            return false;    
        }      
    ...
    */
    public function register_form( $form_id ) {
        ...
        update_option( 'trainingtools_formvalidation', $this->form_val_arr );
    }
    ...
    */
    public function deregister_form( $form_id ) {
        ...
        update_option( 'trainingtools_formvalidation', array() );
    }
    ...
    */
    ...
    /**
    * Validates the values for individual form inputs and stop further 
    * processing of request if a value does not pass.
    * 
    * The validation value is an array of arrays. Each rule must be a key with
    * it's own array which can hold attributes for the validation method being
    * called.
    * 
    * @author Ryan R. Bayne
    * @package Training Tools
    ...
    public function apply_input_validation( $formid ) {
        ...
        foreach( $this->form_val_arr[ $user_ID ][ $formid ] as $input_name => $input_array ) { 
            ...
            if( isset( $input_array['required']) && $input_array['required'] === true ) {
                ...
                if( $input_array['inputtype'] === 'checkboxes' ) {
                    ...
                    if( $box_checked === 0 ) {
                        $this->UI->create_notice( sprintf( __( 'Oops! You missed something. Please make selection for %s as it is a required option.', 'trainingtools' ), $input_array['optiontitle'] ), 'error', 'Small', __( 'Required Option', 'trainingtools' ) );                    
                        return false;                    
                    }                    
                    
                    // if not enough checked FAIL
                     if( $box_checked < $input_array['minimumchecks'] ) {
                        $this->UI->create_notice( sprintf( __( 'You have not checked enough boxes for the %s option.', 'trainingtools' ), $input_array['optiontitle'] ), 'error', 'Small', __( 'Check More Boxes', 'trainingtools' ) );                    
                        return false;                    
                    }
                                       
                    // if too many checked....you guessed it FAIL
                    if( $box_checked > $input_array['maximumchecks'] ) {
                        $this->UI->create_notice( sprintf( __( 'You have checked too many boxes for the %s option.', 'trainingtools' ), $input_array['optiontitle'] ), 'error', 'Small', __( 'Please Uncheck Boxes', 'trainingtools' ) );                    
                        return false;                    
                    }                    
                    ...
                    $this->UI->create_notice( sprintf( __( 'Form incomplete because the %s is a required option.', 'trainingtools' ), $input_array['optiontitle'] ), 'error', 'Small', __( 'Required Option', 'trainingtools' ) );                    
                    return false;                      
                }
            }           
        }
        ...
        foreach( $_POST as $name => $value ) {
            ...
            if( $validation_methods_array !== false && is_array( $validation_methods_array ) ) {
                ...
                foreach( $validation_methods_array as $validation_method => $advanced_validation_atts ) { 
                    
                    // if $validation_method is numeric then it's not a valid method
                    // but if $advanced_validation_atts is a string we will assume the user
                    // has passed a string instead of array and use the string as validation method
                    if( is_numeric( $validation_method ) && is_string( $advanced_validation_atts ) ) {
                        $validation_method = $advanced_validation_atts;
                        $advanced_validation_atts = array();    
                    }
                    ...
                    if( method_exists( $this, 'validate_' . $validation_method ) ) { 
                        ...
                        eval( '$result = self::validate_' . $validation_method . '( $value, $this->form_val_arr[ $user_ID ][ $formid ][ $name ][ "optiontitle" ], $advanced_validation_atts );');// $result is always boolean    
    ...
    */
    public function apply_input_security() {
        ...
        if( !isset( $_POST['trainingtools_form_formid'] ) ) { 
            return false;
        }
        
        $form_id = $_POST['trainingtools_form_formid'];
        ...
        foreach( $_POST as $input_name => $value ) {
            ...
            if( isset( $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ] ) ) {
                ...
                if( isset( $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['disabled'] ) && $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['disabled'] === true ) {
                    ...
                    $this->UI->create_notice( __( 'Possible security violation detected. Please discontinue your action. This event has been logged for investigation.', 'trainingtools' ), 'warning', 'Small', __( 'Warning', 'trainingtools' ) );                    
                    return false;    
                }
            }    
        }                  
            ...
            foreach( $this->form_val_arr[ $user_ID ][ $form_id ] as $input_name => $input_array ) {
                ...
                if( !in_array( $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['inputtype'], $this->grouped_fields ) ) {
                    ...
                    if( isset( $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['disabled'] ) && $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['disabled'] === false ) {
                        ...
                        if( !isset( $_POST[ $input_name ] ) ) {   
                            $this->UI->create_notice( __( 'Possible security violation. Please discontinue your action. This event has been logged for investigation.', 'trainingtools' ), 'warning', 'Small', __( 'Warning', 'trainingtools' ) );                    
                            return false;                        
                        } 
                    }
                }
                ...
                if( $input_array['inputtype'] == 'menu' ) {
                    ...
                    if( $found_match === false ) {
                        $this->UI->create_notice( __( 'Training Tools has security related concerns about a menu on the form you submitted. Please discontinue your action and contact support. This event has been logged for investigation.', 'trainingtools' ), 'warning', 'Small', __( 'Warning', 'trainingtools' ) );                    
                        return false;    
                    }
                }                
                ...
                if( $input_array['inputtype'] == 'checkboxes' ) {
                    ...
                    if( isset( $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['groupcheckboxes'] ) && $this->form_val_arr[ $user_ID ][ $form_id ][ $input_name ]['groupcheckboxes'] == false ) {
                        ...
                        if( $failed_to_match === true ) {
                            $this->UI->create_notice( __( 'WTG CSV Exporter security has detected a possible problem with the form you submitted. Please discontinue your action and contact support.', 'trainingtools' ), 'warning', 'Small', __( 'Warning', 'trainingtools' ) );                    
                            return false;    
                        }
                        ...
                        if( isset( $_POST[ $input_name ] ) && is_array( $_POST[ $input_name ] ) ) {
                            ...
                            // if one or more seletions made but no match this indicates values have been tampered with
                            if( $failed_to_match === true ) {
                                $this->UI->create_notice( __( 'WTG CSV Exporter detected problem relating to the security of the form you submitted. Please discuss this on the WebTechGlobal Forum.', 'trainingtools' ), 'error', 'Small', __( 'Something Went Wrong', 'trainingtools' ) );                    
                                return false;    
                            }
                        }
                    }    
                ...
                if( $input_array['inputtype'] == 'radiogroup' ) {
                    ...
                    // if no match, the submitted item value was not registered when the form was parsed
                    if( $found_match === false ) {
                        $this->UI->create_notice( __( 'Training Tools detected a problem and could not continue. Please try again and if it continues contact support.', 'trainingtools' ), 'warning', 'Small', __( 'Warning', 'trainingtools' ) );                    
                        return false;    
                    }
                }               
                ...
                if( $input_array['inputtype'] == 'hidden' ) {
                    ...
                    if( !isset( $_POST[ $input_name ] ) ) {
                       // arriving here indicates hidden input missing, a hack has removed it
                        $this->UI->create_notice( __( 'A security matter has arisen. Please discontinue your action and contact support if this message continues to appear. The event has been logged.', 'trainingtools' ), 'warning', 'Small', __( 'Warning', 'trainingtools' ) );                    
                        return false;                         
                    } else {
                        // if registered value does not match value submitted, hacker has changed it
                        if( $input_array['currentvalue'] !== $_POST[ $input_name ] ) {
                            $this->UI->create_notice( __( 'Training Tools has detected a possible hack attempt. Your activity has been logged. Please discontinue your action and contact support if this message continues to appear without any known reason.', 'trainingtools' ), 'warning', 'Small', __( 'Warning', 'trainingtools' ) );                    
                            return false;                            
                        }
                    }
                }   
    ...
    */    
    public function user_can( $capability = 'activate_plugins' ){
        if(!current_user_can( $capability ) ){
            $this->UI->create_notice( __( 'Your request was rejected as your account does not have the required permission.', 'trainingtools' ), 'success', 'Small', __( 'Not Permitted', 'trainingtools' ) );          
            return false;
        }   
        return true;
    }     
    ...
    */
    public function form_start( $form_id, $form_name, $form_title, $uploader = false ){
        global $trainingtools_page_name;
        ...
        echo '<input type="hidden" name="trainingtools_admin_action" value="true">';
        echo '<input type="hidden" name="trainingtools_hidden_pagename" value="' . $trainingtools_page_name . '">';
        echo '<input type="hidden" name="trainingtools_form_formid" value="' . $form_name . '">';
        echo '<input type="hidden" name="trainingtools_form_name" value="' . $form_name . '">';
        echo '<input type="hidden" name="trainingtools_form_title" value="' . $form_title . '">';        
    }
    ...
    */
    public function validate_alpha( $value, $optiontitle, $atts_array ) {
        if( !ctype_alpha( $value ) ) {
            $this->UI->create_notice( sprintf( __( "The %s input is only for letters, please edit your entry and remove any numbers or special characters.", 'trainingtools' ), $optiontitle ), 'error', 'Small', __( 'Invalid Entry', 'trainingtools' ) );               
            return false;            
        }
        return true;
    }
    ...
    */
    public function validate_alphanumeric( $value, $optiontitle ) {
        if( !ctype_alnum( $value ) ) {   
            $this->UI->create_notice( sprintf( __( "The %s input is for letters or numbers, no special characters allowed.", 'trainingtools' ), $optiontitle ), 'error', 'Small', __( 'Invalid Entry', 'trainingtools' ) );               
            return false;
        }
        return true;
    }
    ...
    */
    public function validate_url( $url, $optiontitle ) {    
        if( !$this->PHP->validate_url( $url ) ) {
            $this->UI->create_notice(sprintf( __( "You did not enter a valid URL in the %s input. Please ensure that all characters match the online URL.", 'trainingtools' ), $optiontitle ), 'error', 'Small', __( 'Invalid Entry', 'trainingtools' ) );               
            return false;
        }        
        return true;
    } 
    ...
    */
    public function validate_numeric( $value, $optiontitle ) {  
        if( !ctype_digit( $value ) ) {
            $this->UI->create_notice( sprintf( __( "Only numbers are meant to be entered into the %s field.", 'trainingtools' ), $optiontitle ), 'error', 'Small', __( 'Invalid Entry', 'trainingtools' ) );               
            return false;
        }        
        return true;    
    }
    ...
    */
    public function validate_maxlength( $str, $optiontitle, $atts_mixed ) {
        if ( strlen( $str ) > $atts_mixed ) {
            $this->UI->create_notice( sprintf( __( "The maximum length for the %s field is %s ", 'trainingtools' ), $optiontitle, $atts_mixed ), 'error', 'Small', __( 'Too Long', 'trainingtools' ) );               
            return false;
        }        
        return true;        
    }    
    ...
    */
    public function validate_minlength( $str, $optiontitle, $atts_mixed ) {
        if ( strlen( $str ) < $atts_mixed ) {
            $this->UI->create_notice( sprintf( __( "The minimum length for the %s field is %s ", 'trainingtools' ), $optiontitle, $atts_mixed ), 'error', 'Small', __( 'Too Short', 'trainingtools' ) );               
            return false;
        }        
        return true;        
    }
    ...
    */
    public function validate_regex( $str, $optiontitle, $atts_mixed ) {
        if ( !preg_match( $atts_mixed[0], $str ) ) {
            $this->UI->create_notice( sprintf( __( "The %s field entry is not the correct format. A specific pattern of characters is required i.e. %s.", 'trainingtools' ), $optiontitle, $atts_mixed[1] ), 'error', 'Small', __( 'Invalid Value', 'trainingtools' ) );               
            return false;
        }        
        return true;    
    }
    ...
    */
    public function input_switch(){
        ...
        <!-- Option Start -->
        <tr valign="top">
            <th scope="row"><?php _e( $this->optiontitle, 'trainingtools' ); ?></th>
            <td>
        ...
        <tr valign="top">
            ...
            <td>                                     
                <select name="<?php echo $this->inputname;?>" id="<?php echo $this->inputid;?>"<?php if( $this->disabled ){ echo ' disabled';} ?>>
                    ...
                    <option value="notselected" <?php echo $selected;?>><?php _e( 'None Selected', 'trainingtools' ); ?></option> 
    ...
    */
    public function input_menu_cronrepeat() {
        ...
        <tr valign="top">
            ...
            <td>            
                <select name="<?php echo $this->inputname;?>" id="<?php echo $this->inputid;?>"<?php if( $this->disabled ){ echo ' disabled';} ?>>
                    <option value="hourly" <?php if( $this->currentvalue == 'hourly' ){ echo $selected; }?>><?php _e( 'Hourly', 'trainingtools' ); ?></option>
                    <option value="twicedaily" <?php if( $this->currentvalue == 'twicedaily' ){ echo $selected; }?>><?php _e( 'Twice Daily', 'trainingtools' ); ?></option>
                    <option value="daily" <?php if( $this->currentvalue == 'daily' ){ echo $selected; }?>><?php _e( 'Daily', 'trainingtools' ); ?></option>
                </select>                  
            </td>
        </tr>
        <!-- Option End --><?php     
    ...
    */
    public function hidden_basic( $form_id, $id, $name, $current_value ) {
        self::input( $form_id, 'hidden', $id, $name, __( 'Hidden Input ' . $id, 'trainingtools' ), __( 'Hidden Input ' . $id, 'trainingtools' ), true, $current_value, array(), array() ); 
    }
