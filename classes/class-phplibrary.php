...
class TRAININGTOOLS_PHP {  
    ...
    */
    public function arrayinfo_set( $array, $line, $function, $file ){
        $array['arrayinfo']['version'] = TRAININGTOOLS_VERSION;
        $array['arrayinfo']['line'] = $line;
        $array['arrayinfo']['function'] = $function;
        $array['arrayinfo']['file'] = $file;
        $array['arrayinfo']['date'] = TRAININGTOOLS::datewp();
        return $array;   
    }  
    ...
    * 2. Faster than trainingtools_count_extension_in_directory() because it returns sooner, important for directory with a lot of files
    * 
    * @returns boolean
    */
    ...
    */
    public function type( $value ){
        ...
        // is email address
        if( TRAININGTOOLS::is_valid_emailaddress( $value ) ){
            return 'email';
        }                  
    ...
    */
    public function months_time_range( $month, $year, $return = 'array', $format = 'time' ){
        ...
        if( $format == 'date' )
        {
            $array['start'] = TRAININGTOOLS::datewp(0, $array['start'] );
        }
        ...
        if( $format == 'date' )
        {    
            $array['end'] = TRAININGTOOLS::datewp(0, $array['end'] );   
        }
