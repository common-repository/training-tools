<?php
/**
 * Members and methods for Training Tools views that use post boxes (eventually all views)
 *
...
abstract class TRAININGTOOLS_View {
    ...
    public function __construct() {
        ...
        $this->Help = $this->TRAININGTOOLS->load_class( 'TRAININGTOOLS_Help', 'class-help.php', 'classes' );
        $this->PHP = $this->TRAININGTOOLS->load_class( 'TRAININGTOOLS_PHP', 'class-phplibary.php', 'classes' );
        $this->UI = $this->TRAININGTOOLS->load_class( 'TRAININGTOOLS_UI', 'class-ui.php', 'classes' );
        ...
        if( isset( $this->help_array[ $page_name ] ) ){
            ...
            if( isset( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pageabout' ] ) ){
                ...
                $screen->add_help_tab( array(
                    'id' => $page_name,
                    'title' => __( 'About', 'trainingtools' ) . ' ' . $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagetitle' ],
                ...
                if( isset( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagereadmoreurl' ] ) && is_string( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagereadmoreurl' ] ) ){
                    $pagereadmeurl = '<p><a href="' . $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagereadmoreurl' ] . '" target="_blank">' . __( 'Documentation', 'trainingtools') . '</a></p>';        
                } 
                               
                if( isset( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagevideourl' ] ) && is_string( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagevideourl' ] ) ){
                    $pagevideourl = '<p><a href="' . $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagevideourl' ] . '" target="_blank">' . __( 'Video', 'trainingtools') . '</a></p>';
                } 
                               
                if( isset( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagediscussurl' ] ) && is_string( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagediscussurl' ] ) ){
                    $pagediscussionurl = '<p><a href="' . $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagediscussurl' ] . '" target="_blank">' . __( 'Support', 'trainingtools') . '</a></p>';
                }
        
                if( isset( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagefaqurl' ] ) && is_string( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagefaqurl' ] ) ){
                    $pagefaqurl = '<p><a href="' . $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagefaqurl' ]  . '" target="_blank">' . __( 'FAQ', 'trainingtools') . '</a></p>';
                }
                                
                // help tab sidebar
                $screen->set_help_sidebar( '
                <p><strong>' . __( 'For more information:', 'trainingtools' ) . '</strong></p>' . $pagereadmeurl . $pagevideourl . $pagediscussionurl . $pagefaqurl );
    ...
    */
    public function setup( $action, array $data ) {
        ...
        $this->TRAININGTOOLS->enqueue_script( 'common', array( 'jquery', 'postbox' ), array() );
    ...
    */
    protected function add_text_box( $id, $callback, $context = 'normal', $wrap = false ) {
        ...
        $long_id = "trainingtools_{$this->action}-{$id}";
    ...
    */
    protected function add_meta_box( $id, $title, $callback, $context = 'normal', $priority = 'default', $callback_args = null ) {
        ...
        $id = "trainingtools_{$this->action}-{$id}";         
    ...
    */
    protected function action_nonce_field( array $data, array $box ) {
        wp_nonce_field( $this->TRAININGTOOLS->nonce( $this->action ) ); echo "\n";
    }
    ...
    */
    protected function action_field( array $data, array $box ) {
        echo "<input type=\"hidden\" name=\"action\" value=\"trainingtools_{$this->action}\" />\n";
    }
    ...
    */       
    public function render() { 
        ...
        // get the admin page name (slug in menu array, without prepend "trainingtools_")
        $admin_page = $this->TRAININGTOOLS->get_admin_page_name();
        
        // if we are on the main page change $admin_page to 'main' as that is what we use in array
        if( $admin_page === 'trainingtools' ){
            $admin_page = 'main';
        }   
         
        // view header - includes notices output and some admin side automation such as conflict prevention
        $this->TRAININGTOOLS->pageheader( $trainingtools_menu_array[ $admin_page ]['title'], 0);
                               
        // create tab menu for the giving page if the section has two or more pages
        if( !isset( $trainingtools_menu_array[ $admin_page ]['tabmenu'] ) || $trainingtools_menu_array[ $admin_page ]['tabmenu'] === true ) {
            if( $admin_page !== 'main' ) {
                $this->TRAININGTOOLS->build_tab_menu( $admin_page );
            }
        }
    ...
    */
    protected function textbox_submit_button( array $data, array $box ) {
        $caption = isset( $data['submit_button_caption'] ) ? $data['submit_button_caption'] : __( 'Save Changes', 'trainingtools' );
        ?>
